package com.gofield.common.model;

public class Constants {
    public static final String METHOD = "CARD";
    public static final String BASE_PACKAGE = "com.gofield";
    public static final String TOKEN_ISSUER = "www.readys.co.kr";
    public static final String CDN_URL = "https://d22b51gh5o1q1e.cloudfront.net/";
    public static final String RESIZE_ADMIN = "?s=180x180&q=80&f=webp&t=crop";
    public static final String TRACKER_DELIVERY_URL = "https://tracker.delivery/#/%s/%s";
    public static final String AUTHORIZATION = "Authorization";
    public static final String AUTH_PREFIX = "Readys";
    public static final String USER_ID = "userId";
    public static final double SAFE_PAYMENT_CHARGE = 2;
}
