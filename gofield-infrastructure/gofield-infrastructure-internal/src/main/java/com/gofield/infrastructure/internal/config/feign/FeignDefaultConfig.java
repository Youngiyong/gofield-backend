    ...
    private static class FeignErrorDecoder implements ErrorDecoder {
        ...
        public Exception decode(String methodKey, Response response) {
            if (isClientError(response.status())) {
                return new InvalidException(String.format("내부 API 호출 중 클라이언트 에러(%s)가 발생하였습니다. message: (%s)", response.status(), response.body()));
            }
            return new BadGatewayException(String.format("내부 API 연동 중 서버 에러(%s)가 발생하였습니다. message: (%s)", response.status(), response.body()));
        }
